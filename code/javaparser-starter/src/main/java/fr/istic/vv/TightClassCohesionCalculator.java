...
public class TightClassCohesionCalculator extends VoidVisitorWithDefaults<Void> {
    ...
    public void visitTypeDeclaration(TypeDeclaration<?> declaration, Void arg) {
        ...
        StringBuilder graphe = new StringBuilder("strict graph {");

        for (int i = 0; i < lesMethodes.size(); i++) {
            graphe.append(lesMethodes.get(i).getNameAsString()).append(";");
            ...
            for (int j = i + 1; j < lesMethodes.size(); j++) {
                ...
                if (hasSharedAttributes(lesMethodes.get(i), lesMethodes.get(j), lesAttributs)) {
                    somme++;
                    graphe.append(lesMethodes.get(i).getNameAsString()).append(" -- ").append(lesMethodes.get(j).getNameAsString()).append(";");
                }
            }
        }
        ...
        float NP =  N > 1 ? N * (N - 1) / 2.0f : 1; // Calculate combinations of methods (N choose 2), if there is one method set NP to 1

        writer.println(declaration.getFullyQualifiedName().toString() + " : " + somme / NP);

        writer.println(graphe);
